...
//!
//! Runs Phase 2 verification (IPC, spawn, VMO transfer, port aggregation),
//! then performs Phase 5 VFS setup: spawns ramfs and devmgr servers, mounts
//! `/` and `/dev`, and verifies with a directory listing.
...
fn main() -> ! {
    ...
    // ── Phase 5: VFS setup ───────────────────────────────────────────
    debug_log("userboot: starting Phase 5 VFS setup\n");
    setup_vfs();

    debug_log("Phase 5 verification PASSED!\n");
...
fn setup_vfs() {
    ...
    let mut entries_buf = [0u8; 1056]; // 4 entries * 264 bytes each
