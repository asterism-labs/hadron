...
#[cfg(target_os = "none")]
fn create_ramfs() -> alloc::sync::Arc<dyn hadron_kernel::fs::FileSystem> {
    alloc::sync::Arc::new(RamFs::new())
}

#[cfg(target_os = "none")]
hadron_kernel::virtual_fs_entry!(
    RAMFS_ENTRY,
    hadron_kernel::driver_api::registration::VirtualFsEntry {
        name: "ramfs",
        create: create_ramfs,
    }
);
