...
pub unsafe fn enable() {
    // SAFETY: Caller guarantees IDT is valid and enabling interrupts is safe.
    unsafe {
...
#[inline]
pub fn hlt() {
    // SAFETY: HLT is always valid in ring 0; CPU resumes on next interrupt.
    unsafe {
...
#[inline]
pub fn int3() {
    // SAFETY: INT3 is always valid; triggers #BP which is handled by the IDT.
    unsafe {
    ...
    F: FnOnce() -> R,
{
    ...
    let result = f();
    if was_enabled {
        // SAFETY: Interrupts were previously enabled, so re-enabling is valid.
        unsafe { enable() };
    }
    result
}
