...
use super::dyn_dispatch::DynBlockDevice;
...
pub trait KernelServices: Send + Sync {
    ...
    /// Registers a framebuffer device with the kernel's device registry.
    ///
    /// Called by display drivers during probe to make their framebuffer
    /// accessible to the kernel's logging and display subsystems.
    fn register_framebuffer(&self, name: &str, fb: Arc<dyn Framebuffer>);

    /// Registers a block device with the kernel's device registry.
    ///
    /// Called by storage drivers (AHCI, VirtIO) during probe to make their
    /// disks available for filesystem mounting.
    fn register_block_device(&self, name: &str, dev: Box<dyn DynBlockDevice>);

    /// Spawns an async task on the kernel's executor.
    ///
    /// Drivers use this to create long-running I/O tasks (e.g., serial echo)
    /// during probe without depending on kernel internals.
    fn spawn_task(
        &self,
        name: &'static str,
