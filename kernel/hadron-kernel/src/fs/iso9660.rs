...
pub struct Iso9660Fs<D: BlockDevice> {
    ...
    image: Arc<IsoImage<BlockDeviceAdapter<D>>>,
}

impl<D: BlockDevice> Iso9660Fs<D> {
    /// Mount an ISO 9660 image from the given block device.
    ///
    /// # Errors
    ///
...
impl<D: BlockDevice + 'static> FileSystem for Iso9660Fs<D> {
...
struct Iso9660DirInode<D: BlockDevice> {
...
unsafe impl<D: BlockDevice> Send for Iso9660DirInode<D> {}
unsafe impl<D: BlockDevice> Sync for Iso9660DirInode<D> {}

impl<D: BlockDevice + 'static> Inode for Iso9660DirInode<D> {
...
struct Iso9660FileInode<D: BlockDevice> {
    /// Reference to the ISO image for I/O operations.
    image: Arc<IsoImage<BlockDeviceAdapter<D>>>,
...
unsafe impl<D: BlockDevice> Send for Iso9660FileInode<D> {}
unsafe impl<D: BlockDevice> Sync for Iso9660FileInode<D> {}

impl<D: BlockDevice + 'static> Inode for Iso9660FileInode<D> {
    fn inode_type(&self) -> InodeType {
        InodeType::File
    }
