//! Thread-local mocks for interrupt and CPU-local state under Shuttle.
//!
//! Shuttle cannot model hardware interrupts or per-CPU storage. These mocks
//! provide thread-local stand-ins so that [`IrqSpinLock`](super::IrqSpinLock)
//! shuttle tests can verify:
//!
//! - Interrupts are disabled while the lock is held
//! - Flag save/restore is correct on drop
//! - Nested acquisition correctness
//!
//! Each shuttle thread gets its own independent interrupt state, simulating
...
#[allow(dead_code)] // Future: used by per-CPU shuttle tests
