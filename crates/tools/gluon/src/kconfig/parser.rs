...
impl Parser {
    ...
    fn parse_binding(&mut self) -> Result<Binding, String> {
        ...
        match name.as_str() {
            ...
            _ => Err(format!(
                "unknown binding type '{name}', expected cfg, const, or build"
            )),
        }
    }
        ...
        loop {
            ...
            let item = match self.peek_kind() {
                ...
                _ => {
                    let tok = self.peek();
                    return Err(format!(
                        "{}: expected string or identifier in list, got {:?}",
                        tok.span, tok.kind
                    ));
                }
            };
...
"#,
    ...
    #[test]
    fn parse_source() {
        let file = parse_str(r#"source "kernel/kernel/Kconfig""#);
        assert_eq!(file.items.len(), 1);
        assert!(matches!(&file.items[0], KconfigItem::Source(p) if p == "kernel/kernel/Kconfig"));
    }
