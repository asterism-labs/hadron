...
pub fn load_kconfig_tree(root_path: &Path, kconfig_path: &str) -> Result<KconfigFile, String> {
...
fn resolve_sources(items: Vec<KconfigItem>, root_path: &Path) -> Result<Vec<KconfigItem>, String> {
...
pub fn load_kconfig(
    ...
    (
        BTreeMap<String, ConfigOptionDef>,
        Vec<String>,
        BTreeMap<String, PresetDef>,
        Vec<PathBuf>,
    ),
    String,
> {
    ...
    crate::verbose::vprintln!(
        "  kconfig: {} options, {} presets from {} files",
        options.len(),
        presets.len(),
        loaded_files.len()
    );
    ...
    let (config_type, default, choices) = match ty_decl.kind {
        ...
        TypeKind::U32 => {
            let default = match &block.default {
                ...
                _ => {
                    return Err(format!(
                        "config '{}': u32 expects integer default",
                        block.name
                    ));
                }
            };
        ...
        TypeKind::U64 => {
            let default = match &block.default {
                ...
                _ => {
                    return Err(format!(
                        "config '{}': u64 expects integer default",
                        block.name
                    ));
                }
            };
        ...
        TypeKind::Str => {
            let default = match &block.default {
                ...
                _ => {
                    return Err(format!(
                        "config '{}': str expects string default",
                        block.name
                    ));
                }
            };
        ...
        TypeKind::Choice => {
            ...
            if variants.is_empty() {
                return Err(format!(
                    "config '{}': choice requires variant list",
                    block.name
                ));
            }
            let default = match &block.default {
                ...
                _ => {
                    return Err(format!(
                        "config '{}': choice expects string default",
                        block.name
                    ));
                }
            };
...
mod tests {
    ...
    fn convert_preset_block_basic() {
        let block = PresetBlock {
            ...
            overrides: vec![
                PresetOverride {
                    name: "lock_debug".to_string(),
                    value: DefaultValue::Bool(true),
                },
                PresetOverride {
                    name: "LOG_LEVEL".to_string(),
                    value: DefaultValue::Str("debug".to_string()),
                },
                PresetOverride {
                    name: "MAX_CPUS".to_string(),
                    value: DefaultValue::Integer(4),
                },
            ],
        };
        ...
        assert!(matches!(
            preset.overrides.get("lock_debug"),
            Some(ConfigValue::Bool(true))
        ));
        assert!(
            matches!(preset.overrides.get("LOG_LEVEL"), Some(ConfigValue::Str(s)) if s == "debug")
        );
        assert!(matches!(
            preset.overrides.get("MAX_CPUS"),
            Some(ConfigValue::U32(4))
        ));
    }
    ...
    fn convert_preset_large_integer_to_u64() {
        ...
        let preset = convert_preset_block(&block);
        assert!(matches!(
            preset.overrides.get("BIG_VALUE"),
            Some(ConfigValue::U64(0x1_0000_0000))
        ));
    }
    ...
    fn resolve_sources_inlines_and_recurses_menus() {
        ...
        let items = vec![
            ...
            KconfigItem::Menu(MenuBlock {
                title: "Sub".to_string(),
                items: vec![KconfigItem::Config(ConfigBlock {
                    name: "B".to_string(),
                    ty: Some(TypeDecl {
                        kind: TypeKind::U32,
                        variants: vec![],
