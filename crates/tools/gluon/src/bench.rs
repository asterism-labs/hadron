...
//!
//! Discovers benchmark files in `kernel/kernel/benches/`, compiles each
...
/// - Discovers `.rs` files in `kernel/kernel/benches/`
...
fn compile_kernel_benchmarks(
    ...
    let benches_dir = state
        ...
        .benches_dir
        .as_deref()
        .unwrap_or("kernel/kernel/benches");
...
fn compile_bench_binary(
    ...
    if !binary.exists() {
        bail!("expected benchmark binary not found: {}", binary.display());
    }
...
fn run_benchmark_binary(
    ...
    let (result, serial_data) = run::run_with_serial_capture(config, kernel_binary, extra_args)?;
