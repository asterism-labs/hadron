...
fn load_model_inner(root: &PathBuf, validate: bool, force: bool) -> Result<model::BuildModel> {
    ...
    if !model.dependencies.is_empty() {
        ...
        let resolved = vendor::resolve_transitive(
            &model.dependencies,
            &vendor_dir,
            &mut version_cache,
            &locked_versions,
...
fn resolve_config(cli: &cli::Cli) -> Result<(config::ResolvedConfig, model::BuildModel)> {
    ...
    vprintln!(
        "  resolving config: profile={}, target={}",
        profile_name,
        cli.target.as_deref().unwrap_or("(from profile)")
    );
    let resolved = config::resolve_from_model(&model, profile_name, cli.target.as_deref(), &root)?;
    Ok((resolved, model))
}
...
fn cmd_vendor(cli: &cli::Cli, args: &cli::VendorArgs) -> Result<()> {
    ...
    if args.check {
            ...
            match lock.packages.iter().find(|p| p.name == new_pkg.name) {
                Some(existing) => {
                    ...
                    if existing.checksum != new_pkg.checksum {
                        println!("  MISMATCH: {} checksum differs", new_pkg.name);
                        mismatches += 1;
                    }
                }
                None => {
        ...
        for existing in &lock.packages {
            if !new_lock.packages.iter().any(|p| p.name == existing.name) {
                println!(
                    "  EXTRA: {} in lock file but not in dependencies",
                    existing.name
                );
                mismatches += 1;
            }
    ...
    let crates_io_names: Vec<String> = model
        .dependencies
        .iter()
                        ...
                        .map(|dep| {
                            ...
                            s.spawn(move || -> Result<PathBuf> {
                                match &dep.source {
                                    ...
                                    vendor::ResolvedSource::Path { .. } => Ok(vdir.join(&dep.name)),
                                }
                            })
                        })
                        .collect();
