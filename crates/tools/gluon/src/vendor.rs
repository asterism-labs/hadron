    ...
    while let Some(entry) = queue.pop_front() {
        ...
        for cargo_dep in &parsed.dependencies {
            ...
            let resolved_version =
                if !raw_version.is_empty() && matches!(source, ResolvedSource::CratesIo) {
...
fn versions_compatible(a: &str, b: &str) -> bool {
    let Ok(va) = semver::Version::parse(a) else {
        return false;
    };
    let Ok(vb) = semver::Version::parse(b) else {
        return false;
    };
...
/// self-contained.
fn resolve_workspace_references(member: &mut toml::Table, meta: &WorkspaceMetadata) {
...
fn resolve_deps_workspace_refs(member: &mut toml::Table, section: &str, ws_deps: &toml::Table) {
...
fn fetch_version_listing(name: &str) -> Result<(String, Vec<CrateVersionEntry>)> {
    ...
    let entries: Vec<CrateVersionEntry> = versions
        .iter()
        .filter_map(|v| {
...
pub fn prefetch_versions(
    ...
    for chunk in missing.chunks(batch_size) {
        let results: Vec<Result<(String, Vec<CrateVersionEntry>)>> = std::thread::scope(|s| {
            let handles: Vec<_> = chunk
                .iter()
                .map(|name| s.spawn(move || fetch_version_listing(name)))
                .collect();
...
pub fn resolve_version(name: &str, version_str: &str, cache: &mut VersionCache) -> Result<String> {
    ...
    let req: semver::VersionReq = version_str
        .parse()
    ...
    match best {
        ...
        None => bail!("no non-yanked version of '{name}' matches requirement '{version_str}'"),
    }
}
...
pub fn fetch_git(
    ...
    let short_ref = if reference.len() > 8 {
        &reference[..8]
    } else {
        reference
    };
                                        ...
                                        if member_name == Some(name) {
                                            ...
                                            if let Ok(raw) = std::fs::read_to_string(&dest_cargo) {
                                                    ...
                                                    raw.parse::<toml::Table>()
                                                {
                                                    ...
                                                    let normalized = toml::to_string_pretty(
                                                        &member_doc,
                                                    )
...
pub fn ensure_vendored(
    ...
    if vendor_dir.exists() {
        ...
        if cleaned > 0 {
            println!(
                "  Cleaned up {cleaned} corrupt vendor director{}",
    ...
    let crates_io_names: Vec<String> = dependencies
        .iter()
    ...
    for iteration in 1..=max_iterations {
        ...
        let to_fetch: Vec<&ResolvedDep> = resolved
            .iter()
            .filter(|dep| {
                let vendor_path = find_vendor_dir(&dep.name, Some(&dep.version), &vendor_dir);
        ...
        if iteration == max_iterations {
            anyhow::bail!(
                "transitive resolution did not converge after {max_iterations} iterations"
            );
        }
                    ...
                    .map(|dep| {
                        ...
                        s.spawn(move || -> Result<std::path::PathBuf> {
                            match &dep.source {
                                ...
                                ResolvedSource::Path { .. } => Ok(vdir.join(&dep.name)),
                            }
                        })
                    })
                    .collect();
...
pub fn write_lock_file(path: &Path, lock: &LockFile) -> Result<()> {
    ...
    let content = toml::to_string_pretty(lock).context("serializing lock file")?;
...
pub fn build_lock_file(resolved: &[ResolvedDep], vendor_dir: &Path) -> Result<LockFile> {
    ...
    for dep in resolved {
        ...
        let dependencies = if vendor_path.join("Cargo.toml").exists() {
            ...
            parsed
                .map(|p| {
                    let activated =
                        compute_activated_features(&dep.features, &p.features, &p.default_features);
                    p.dependencies
                        .iter()
                        .filter(|d| {
                            if d.optional {
                                let dep_feat_key = format!("dep:{}", d.key);
                                activated.contains(&d.key.to_string())
...
pub fn auto_register_dependencies(
    ...
    let resolved_map: HashMap<&str, &ResolvedDep> =
    ...
    for dep in resolved {
        ...
        let activated =
            compute_activated_features(&dep.features, &parsed.features, &parsed.default_features);
...
mod tests {
    ...
    fn make_test_dir(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("gluon_vendor_test_{}_{}", std::process::id(), name));
