...
pub fn build_sysroot(
    ...
    let lib_dir = sysroot_dir
        ...
        .join(target_name)
        .join("lib");
    std::fs::create_dir_all(&lib_dir).context("failed to create sysroot output directory")?;

    let target_flag = target_spec.to_str().context("non-UTF-8 target spec path")?;
    ...
    let cb_edition = detect_edition(&sysroot_src.join("compiler-builtins/compiler-builtins"))?;
