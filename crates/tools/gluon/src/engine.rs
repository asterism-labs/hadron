...
fn register_config_api(engine: &mut Engine, model: SharedModel) {
    ...
    engine.register_fn(
        "config_bool",
        move |name: &str, default: bool| -> ConfigBuilder {
            let mut model = m.lock().unwrap();
    ...
    engine.register_fn(
        "config_u32",
        move |name: &str, default: i64| -> ConfigBuilder {
            let mut model = m.lock().unwrap();
            model.config_options.insert(
                name.into(),
                ConfigOptionDef {
    ...
    engine.register_fn(
        "config_u64",
        move |name: &str, default: &str| -> ConfigBuilder {
    ...
    let m = model.clone();
    engine.register_fn("config_group", move |name: &str| -> ConfigGroupBuilder {
        let mut model = m.lock().unwrap();
        model.config_options.insert(
            name.into(),
            ConfigOptionDef {
...
fn register_group_api(engine: &mut Engine, model: SharedModel) {
    ...
    builder_method!(engine, "deps", CrateBuilder, |builder, model, deps: Map| {
    ...
    builder_method!(
        ...
        |builder, model, name: &str| {
            model
                .pipeline
                .steps
                .push(PipelineStep::Barrier(name.into()));
        }
    );
    ...
    engine
        .register_custom_syntax(
            ...
            move |context, inputs| {
                ...
                let rel_path: String = path_val.into_string().map_err(|e| {
