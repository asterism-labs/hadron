...
pub fn resolve_group_from_model(
    ...
    if sorted_names.len() != matching.len() {
        bail!("dependency cycle detected among crates in group '{group_name}'");
    }
                ...
                if let Some(def) = model.crates.get(&krate.name) {
                    ...
                    for req in &def.requires_config {
                        match config_options.get(req) {
                            ...
                            _ => {
                                println!(
                                    "  Skipping {} (requires config '{req}' which is disabled)",
                                    krate.name
                                );
                                gated = true;
                                break;
...
mod tests {
    ...
    fn linear_chain_sorted_correctly() {
        ...
        let model = make_model(&[("a", &["b"]), ("b", &["c"]), ("c", &[])]);
        let names = resolve_names(&model).expect("resolution should succeed");
    ...
    fn diamond_dependency() {
        ...
        let model = make_model(&[("a", &["b", "c"]), ("b", &["d"]), ("c", &["d"]), ("d", &[])]);
        let names = resolve_names(&model).expect("resolution should succeed");
    ...
    fn resolved_crate_fields_are_correct() {
        ...
        assert_eq!(
            rc.root_file,
            PathBuf::from("/fake/root/crates/foo/src/lib.rs")
        );
    ...
    fn three_node_cycle_detected() {
        ...
        let model = make_model(&[("a", &["b"]), ("b", &["c"]), ("c", &["a"])]);
        let err = resolve_names(&model).expect_err("cycle should produce an error");
        let msg = err.to_string();
        assert!(
            msg.contains("cycle"),
