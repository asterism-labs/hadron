...
fn compile_crate_cross(
    ...
    let out_dir = config.root.join("build/kernel").join(suffix).join("debug");
    std::fs::create_dir_all(&out_dir)?;
    ...
    let verb = if mode == CompileMode::Clippy {
        "lint"
    } else {
        "compile"
    };
    ...
    let artifact = if !is_check && krate.crate_type == CrateType::Bin {
        ...
        out_dir.join(format!("lib{}.rmeta", crate_name_sanitized(&krate.name)))
    } else {
        out_dir.join(format!("lib{}.rlib", crate_name_sanitized(&krate.name)))
    };
...
fn compile_crate_host(
    ...
    let artifact = if crate_type == "proc-macro" {
        ...
        out_dir.join(format!("lib{}.{ext}", crate_name_sanitized(&krate.name)))
    } else {
        out_dir.join(format!("lib{}.rlib", crate_name_sanitized(&krate.name)))
    };
...
fn host_sysroot_lib_dir() -> Result<PathBuf> {
    ...
    let sysroot = String::from_utf8(sysroot_output.stdout)?.trim().to_string();
...
pub fn crate_out_dir(
    ...
    if krate.target == "host" {
        ...
        let suffix = out_dir_suffix.unwrap_or(&krate.target);
        project_root.join("build/kernel").join(suffix).join("debug")
    }
}
