        ...
        if let Some(dot_pos) = name.find('.') {
            let prefix = &name[..dot_pos];
                ...
                model
                    .config_options
                    .get(prefix)
                    .is_some_and(|o| o.ty == ConfigType::Group),
...
fn validate_crates(model: &BuildModel) -> Result<()> {
    for (name, krate) in &model.crates {
        ensure!(!krate.path.is_empty(), "crate '{name}' has no path");
    ...
    for target in &targets_seen {
        ...
        let mut in_degree: std::collections::HashMap<&str, usize> =
            target_crates.iter().map(|&n| (n, 0)).collect();
...
fn validate_dependencies(model: &BuildModel) -> Result<()> {
    for (name, dep) in &model.dependencies {
        ensure!(!name.is_empty(), "dependency has an empty name");
...
mod tests {
    ...
    use crate::model::{
        ...
        DepSource, ExternalDepDef, GitRef, GroupDef, PipelineStep, PresetDef, ProfileDef, RuleDef,
    ...
    fn crate_undefined_dep_fails() {
        ...
        krate
            .deps
            .insert("nonexistent".into(), make_dep("nonexistent"));
    ...
    fn preset_inheritance_cycle_detected() {
        ...
        model
            .config_options
            .insert("opt".into(), bool_opt("opt", false));
    ...
    fn valid_model_with_presets() {
        ...
        model
            .config_options
            .insert("serial_log".into(), bool_opt("serial_log", true));
        model
            .config_options
            .insert("lock_debug".into(), bool_opt("lock_debug", false));
