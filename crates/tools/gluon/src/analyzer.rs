...
pub fn generate_rust_project(config: &ResolvedConfig, model: &BuildModel) -> Result<PathBuf> {
    ...
    let non_host_group_names: Vec<String> = model
        .groups
        .iter()
    ...
    for gname in &non_host_group_names {
        let group_crates =
            crate_graph::resolve_group_from_model(model, gname, &config.root, &sysroot_src)?;
        for krate in group_crates {
            if !non_host_crates
                .iter()
                .any(|k: &ResolvedCrate| k.name == krate.name)
            {
    ...
    let project_crate_names: Vec<String> = model
        .crates
        .iter()
    ...
    let config_groups: std::collections::HashSet<String> = model
        .groups
        .iter()
        .filter(|(_, g)| g.config)
        .map(|(name, _)| name.clone())
    ...
    for krate in &all_crates {
        ...
        let crate_group = model.crates.get(&krate.name).and_then(|c| c.group.as_ref());
    ...
    let json = serde_json::to_string_pretty(&project).context("serializing rust-project.json")?;
...
fn resolve_groups_by_target(
    ...
    let group_names: Vec<String> = model
        .groups
        .iter()
...
fn build_config_cfgs(config: &ResolvedConfig) -> Vec<String> {
    ...
    for (name, value) in &config.options {
        ...
        let has_cfg_cumulative =
            opt_bindings.map_or(false, |bs| bs.contains(&Binding::CfgCumulative));
