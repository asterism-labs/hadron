                ...
                if is_enabled {
                    for selected in &opt.selects {
                        if let Some(ConfigValue::Bool(false)) = self.values.get(selected) {
                            self.values
                                .insert(selected.clone(), ConfigValue::Bool(true));
                            changed = true;
                        }
                    }
                }
...
fn draw_breadcrumb(f: &mut ratatui::Frame, app: &App, area: Rect) {
    ...
    let widget = Paragraph::new(format!(" {breadcrumb}")).style(
        Style::default()
            .fg(Color::Cyan)
...
fn draw_help_popup(f: &mut ratatui::Frame, app: &App, popup: &HelpPopupState, area: Rect) {
