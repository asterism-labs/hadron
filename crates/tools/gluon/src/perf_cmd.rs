...
fn cmd_perf_report(args: &PerfReportArgs) -> Result<()> {
    ...
    let resolver =
        perf::symbol_resolver::SymbolResolver::from_kernel_elf(kernel_path, results.kernel_vbase);
    ...
    match args.mode.as_str() {
        ...
        "folded" => {
            let output = args
                .output
                .as_deref()
                .ok_or_else(|| anyhow::anyhow!("folded mode requires --output (-o) path"))?;
...
fn cmd_perf_record(args: &PerfRecordArgs, config: Option<&ResolvedConfig>) -> Result<()> {
    let config = config
        .ok_or_else(|| anyhow::anyhow!("perf record requires a resolved build configuration"))?;
    ...
    println!("Recording profiling data from {}...", kernel_path.display());
