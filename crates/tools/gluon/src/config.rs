//! Configuration resolution for gluon.
//!
...
/// Find the project root by looking for `gluon.rhai`.
pub fn find_project_root() -> Result<PathBuf> {
    let mut dir = std::env::current_dir()?;
    loop {
        if dir.join("gluon.rhai").exists() {
            return Ok(dir);
        }
        if !dir.pop() {
            bail!("could not find gluon.rhai in any parent directory");
        }
    }
}
...
pub fn resolve_from_model(
    ...
    let options = resolve_options(&model.config_options, &merged_overrides)?;

    // Apply select/depends-on validation.
    let options = apply_selects_and_validate(options, &model.config_options)?;
    ...
    let tests = TestsConfig {
        ...
        shuttle_testable: model.tests.shuttle_testable.clone(),
    ...
    let bindings: BTreeMap<String, Vec<crate::model::Binding>> = model
        .config_options
        .iter()
    ...
    let choices: BTreeMap<String, Vec<String>> = model
        .config_options
        .iter()
        .filter_map(|(name, opt)| opt.choices.as_ref().map(|c| (name.clone(), c.clone())))
        .collect();
...
fn collect_profile_config(model: &BuildModel, name: &str) -> Result<BTreeMap<String, ConfigValue>> {
    let profile = model
        .profiles
        .get(name)
...
fn resolve_preset(model: &BuildModel, name: &str) -> Result<BTreeMap<String, ConfigValue>> {
    let preset = model
        .presets
        .get(name)
...
fn resolve_profile(model: &BuildModel, name: &str) -> Result<ResolvedProfile> {
    let profile = model
        .profiles
        .get(name)
        .with_context(|| format!("profile '{name}' not found in build model"))?;
...
fn resolve_options(
    ...
    for (name, opt) in options {
        ...
        let value = profile_overrides.get(name).unwrap_or(&opt.default);
            ...
            for dep in &def.depends_on {
                match options.get(dep) {
                    ...
                    Some(ResolvedValue::Bool(false)) => {
                        bail!("option '{name}' depends on '{dep}', but '{dep}' is disabled");
                    }
                    _ => {
                        ensure!(
...
pub fn load_config_overrides(root: &Path) -> Result<BTreeMap<String, ConfigValue>> {
    ...
    for (lineno, line) in contents.lines().enumerate() {
        ...
        let Some((key, val)) = line.split_once('=') else {
            bail!(
                ".hadron-config:{}: invalid line (expected KEY=VALUE): {line}",
                lineno + 1
            );
        };
        ...
        let config_val = if val == "true" {
            ...
            let parsed = u64::from_str_radix(&hex.replace('_', ""), 16).with_context(|| {
                format!(".hadron-config:{}: invalid hex value: {val}", lineno + 1)
            })?;
...
pub fn save_config_overrides(root: &Path, values: &BTreeMap<String, ConfigValue>) -> Result<()> {
    let path = root.join(".hadron-config");
    let mut content = String::new();
    content.push_str("# Generated by gluon menuconfig\n");
...
mod tests {
    ...
    fn multi_level_profile_inheritance() {
        use crate::model::{BuildModel, ProfileDef, TargetDef};
    ...
    fn u32_range_validation_pass() {
        ...
        assert!(matches!(
            resolved.get("timer_hz"),
            Some(ResolvedValue::U32(50))
        ));
    }
    ...
    fn u32_range_validation_fail() {
        ...
        assert!(
            err_msg.contains("outside range"),
            "expected 'outside range' in error, got: {err_msg}"
        );
    }
    ...
    fn choice_validation_fail() {
        ...
        let err_msg = result.unwrap_err().to_string();
        assert!(
            err_msg.contains("not in choices"),
            "expected 'not in choices' in error, got: {err_msg}"
        );
    }
...
",
    ...
    fn preset_resolution_basic() {
        use crate::model::{BuildModel, PresetDef, ProfileDef, TargetDef};
        ...
        assert!(matches!(
            result.get("serial_log"),
            Some(ConfigValue::Bool(true))
        ));
        assert!(matches!(
            result.get("lock_debug"),
            Some(ConfigValue::Bool(true))
        ));
    }
